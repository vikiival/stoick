...
import {Block, DataSourceBuilder, BlockHeader, SolanaRpcClient, Instruction as _Instruction, Transaction as _Transaction} from '@subsquid/solana-stream'
...
import { ARCHIVE_URL, NODE_URL, PREINDEX_BLOCK, PROGRAM_ID, STARTING_BLOCK, disabledRPC } from './environment'
    ...
    for (let block of blocks) {
        for (let ins of block.instructions) {
            ...
            if (ins.programId === PROGRAM_ID) {
                ...
                debug('tx' as any, { tx: ins.getTransaction().signatures[0] }, true)
    
                switch (discriminator) {
                    // case instructions.createCollectionV1.d8:
                    case 0:
                        const mint = instructions.createV1.decode({ accounts: ins.accounts, data: ins.data })
                        debug('createV1' as any, mint, true)
                        break
                    case 1:
                        const collection = instructions.createCollectionV1.decode({ accounts: ins.accounts, data: ins.data })
                        debug('createCollectionV1' as any, collection, true)
                        break

                    default:
                        console.log('unknown instruction',discriminator, ins.d1, ins.d8)
                        break
                }
